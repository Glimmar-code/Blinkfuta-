...
class SellerService {
  ...
  /// Fetches listings for a given seller ID
  static Future<List<MarketItem>> fetchSellerListings(String sellerId) async {
    try {
      final rows = await _client.from('market_items').select().eq('seller_id', sellerId).order('created_at', ascending: false);
      return rows.map((r) => MarketItem.fromMap(r)).toList();
    } catch (_) {
      return [];
    }
  }
